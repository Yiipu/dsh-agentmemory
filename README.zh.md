...
> **注意：agentmemory 官方的 `connect dsh`。** agentmemory 0.9.29 自带上游 dsh 连接器，与本插件功能重叠：
>
> - **它是什么**：`agentmemory connect dsh` 向 home 级 `cordis.patch.yml` 写入一行 `@deepseek-ai/dsh-mcp-client`；`--with-hooks` 再经 `dsh-hooks-claude-code` 桥接运行其捆绑的 Claude Code hook 脚本。
> - **为何冲突**：两者都捕获会话活动并暴露记忆工具，而 daemon 去重只合并同 session id 下的相同行——同时开启等于双份捕获、部分重复。
> - **每个 profile 二选一**：本插件（标准 hookType 映射、`agent/pre-step` 注入、原生 `memory_recall` / `memory_remember`）或上游连接器。
